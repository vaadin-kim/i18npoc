...
import com.vaadin.i18n.TranslationProvider;
...
public class JsonCodec implements Serializable {
    ...
    public static EncodeResult encode(Object value, JsonValue diffState,
        ...
        if (value instanceof JsonValue) {
        	...
        	if(m.find()) {
        		Connector connector = TranslationProvider.currentConnector.get();
        		if(connector instanceof AbstractComponent) {
        			TranslationProvider provider = ((AbstractComponent) connector).getTranslationProvider();
        			if(provider != null) {
        				String key = m.group(2);
        				value = provider.getTranslation(((AbstractComponent) connector).getLocale(), key);
        			}
        		}
        	}
